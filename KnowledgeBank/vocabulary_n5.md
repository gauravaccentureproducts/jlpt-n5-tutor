# JLPT N5 Vocabulary (Goi)

Exhaustive list of JLPT N5 vocabulary, grouped for readability. Each entry follows the format:

`Japanese (Reading) — English meaning`

## Kanji Usage Rule

To match the N5 learner's actual reading ability, this list uses **only kanji that appear in the N5 kanji syllabus** (see `kanji_n5.md`).

- If every kanji in a word is part of the N5 syllabus, the kanji form is shown (e.g., 学生).
- If a word contains any kanji that is *not* in the N5 syllabus, the entire word is rendered in **hiragana** (or katakana for loanwords) — even if part of the word would otherwise use a known kanji. This avoids "half-known" presentations.
- Loanwords stay in their conventional katakana form.

## Tier Legend

- Items without a tag are **Core N5** — confidently expected at N5.
- **[Ext]** = *Extended / borderline* — listed by some N5 sources but commonly placed at N4 by others. Useful for recognition; do not over-prioritize.
- **[Cul]** = *Cultural / situational* — encountered in everyday Japan but rarely a focus on the N5 exam.

## Variant Policy

When two forms exist (e.g., a Sino-Japanese term plus a katakana loan), the dominant form is listed first, with the alternate noted on the same line. Some words intentionally appear under more than one section (e.g., 名前 under both *Common Nouns* and *Misc Useful Items*) because they belong functionally to multiple thematic groups. This is by design, not duplication.

---

## 1. People — Pronouns and Self

- 私 (わたし) — I, me
- 私たち (わたしたち) — we
- あなた — you
- かれ — he, boyfriend
- かのじょ — she, girlfriend
- かた — person (polite)
- 人 (ひと) — person
- みなさん — everyone (polite)
- だれ — who
- どなた — who (polite)
- じぶん — oneself

## 2. People — Family

- かぞく — family
- 父 (ちち) — father (own)
- 母 (はは) — mother (own)
- お父さん (おとうさん) — father (someone else's / address form)
- お母さん (おかあさん) — mother (someone else's / address form)
- あに — older brother (own)
- あね — older sister (own)
- おとうと — younger brother
- いもうと — younger sister
- おにいさん — older brother (polite)
- おねえさん — older sister (polite)
- きょうだい — siblings
- りょうしん — parents
- そふ — grandfather (own)
- そぼ — grandmother (own)
- おじいさん — grandfather, elderly man
- おばあさん — grandmother, elderly woman
- おじさん — uncle, middle-aged man
- おばさん — aunt, middle-aged woman
- 子ども (こども) — child (often written こども in everyday text)
- 男の子 (おとこのこ) — boy
- 女の子 (おんなのこ) — girl
- 男 (おとこ) — man
- 女 (おんな) — woman
- 大人 (おとな) — adult
- ともだち — friend

## 3. People — Roles

- 学生 (がくせい) — student
- せいと — pupil, schoolchild
- 先生 (せんせい) — teacher, doctor
- いしゃ — doctor
- 会社員 (かいしゃいん) — company employee
- 駅員 (えきいん) — station staff
- 店員 (てんいん) — shop clerk
- けいかん — police officer **[Ext]**
- おまわりさん — police officer (friendly) **[Ext]**
- りゅうがくせい — international student **[Ext]**
- 外国人 (がいこくじん) — foreigner

## 4. Body Parts

- からだ — body
- あたま — head
- かお — face
- め — eye
- みみ — ear
- はな — nose
- くち — mouth
- は — tooth
- て — hand
- あし — leg, foot
- おなか — stomach, belly
- せ — back, height

## 5. Demonstratives (こそあど)

- これ — this (thing near speaker)
- それ — that (thing near listener)
- あれ — that (over there)
- どれ — which (one)
- この — this (modifier)
- その — that (modifier)
- あの — that over there (modifier)
- どの — which (modifier)
- ここ — here
- そこ — there
- あそこ — over there
- どこ — where
- こちら — this way / this person (polite)
- そちら — that way
- あちら — that way (over there)
- どちら — which way
- こっち — this way (casual)
- そっち — that way (casual)
- あっち — that way over there (casual)
- どっち — which way (casual)
- こんな — this kind of
- そんな — that kind of
- あんな — that kind of (over there)
- どんな — what kind of
- こう — like this
- そう — like that
- ああ — like that (over there)
- どう — how

## 6. Question Words

- 何 (なに / なん) — what
- いつ — when
- いくら — how much (price)
- いくつ — how many, how old
- 何時 (なんじ) — what time
- 何曜日 (なんようび) — what day of the week
- 何月 (なんがつ) — what month
- 何日 (なんにち) — what day
- なぜ — why
- どうして — why
- 何で (なんで) — why, by what means

## 7. Numbers

- ゼロ / れい — zero
- 一 (いち) — one
- 二 (に) — two
- 三 (さん) — three
- 四 (し / よん) — four
- 五 (ご) — five
- 六 (ろく) — six
- 七 (しち / なな) — seven
- 八 (はち) — eight
- 九 (きゅう / く) — nine
- 十 (じゅう) — ten
- 十一 (じゅういち) — eleven
- 二十 (にじゅう) — twenty
- 百 (ひゃく) — hundred
- 千 (せん) — thousand
- 万 (まん) — ten thousand
- 一万 (いちまん) — ten thousand
- おく — hundred million **[Ext]**

## 8. Native Counters (つ-series)

- 一つ (ひとつ) — one (thing)
- 二つ (ふたつ) — two
- 三つ (みっつ) — three
- 四つ (よっつ) — four
- 五つ (いつつ) — five
- 六つ (むっつ) — six
- 七つ (ななつ) — seven
- 八つ (やっつ) — eight
- 九つ (ここのつ) — nine
- 十 (とお) — ten
- いくつ — how many

## 9. Counters (Common)

- 人 (にん) — counter for people
- 一人 (ひとり) — one person
- 二人 (ふたり) — two people
- まい — counter for flat objects
- 本 (ほん) — counter for long thin objects
- だい — counter for machines / vehicles
- さつ — counter for books
- ひき — counter for small animals
- はい — counter for cups / glasses
- こ — counter for small objects
- かい — floor (of a building)
- かい — counter for occurrences
- 番 (ばん) — number, ordinal
- ど — degrees, occurrences

## 10. Time — General

- とき — time, when
- 時間 (じかん) — time, hour
- とけい — clock, watch
- 今 (いま) — now
- 今日 (きょう) — today
- あした — tomorrow
- きのう — yesterday
- あさって — day after tomorrow
- おととい — day before yesterday
- あさ — morning
- ひる — noon, daytime
- ゆうがた — evening
- よる — night
- ばん — evening, night
- けさ — this morning
- こんばん — this evening
- こんや — tonight
- 午前 (ごぜん) — A.M.
- 午後 (ごご) — P.M.
- 半 (はん) — half (past)
- 分 (ふん / ぷん) — minute(s)
- びょう — second(s)

## 11. Time — Days, Weeks, Months, Years

- 日 (ひ) — day
- 一日 (ついたち) — 1st of the month
- 二日 (ふつか) — 2nd / two days
- 三日 (みっか) — 3rd / three days
- 四日 (よっか) — 4th / four days
- 五日 (いつか) — 5th / five days
- 六日 (むいか) — 6th / six days
- 七日 (なのか) — 7th / seven days
- 八日 (ようか) — 8th / eight days
- 九日 (ここのか) — 9th / nine days
- 十日 (とおか) — 10th / ten days
- 二十日 (はつか) — 20th / twenty days
- 週 (しゅう) — week
- 先週 (せんしゅう) — last week
- 今週 (こんしゅう) — this week
- 来週 (らいしゅう) — next week
- 毎週 (まいしゅう) — every week
- 月曜日 (げつようび) — Monday
- 火曜日 (かようび) — Tuesday
- 水曜日 (すいようび) — Wednesday
- 木曜日 (もくようび) — Thursday
- 金曜日 (きんようび) — Friday
- 土曜日 (どようび) — Saturday
- 日曜日 (にちようび) — Sunday
- しゅうまつ — weekend
- 月 (つき) — month, moon
- 一月 (いちがつ) — January
- 二月 (にがつ) — February
- 三月 (さんがつ) — March
- 四月 (しがつ) — April
- 五月 (ごがつ) — May
- 六月 (ろくがつ) — June
- 七月 (しちがつ) — July
- 八月 (はちがつ) — August
- 九月 (くがつ) — September
- 十月 (じゅうがつ) — October
- 十一月 (じゅういちがつ) — November
- 十二月 (じゅうにがつ) — December
- 先月 (せんげつ) — last month
- 今月 (こんげつ) — this month
- 来月 (らいげつ) — next month
- 毎月 (まいつき) — every month
- 年 (とし / ねん) — year
- きょねん — last year
- 今年 (ことし) — this year
- 来年 (らいねん) — next year
- 毎年 (まいとし / まいねん) — every year
- おととし — year before last
- さらいねん — year after next **[Ext]**
- たんじょうび — birthday

## 12. Time — Frequency / Sequence

- 毎日 (まいにち) — every day
- まいあさ — every morning
- まいばん — every night
- いつも — always
- よく — often
- 時々 (ときどき) — sometimes
- たまに — occasionally
- あまり — not very (with negative)
- ぜんぜん — not at all (with negative)
- すぐ — soon, immediately
- もう — already, soon, more
- まだ — still, not yet
- はじめて — for the first time
- さいしょ — first, beginning
- さいご — last, end
- つぎ — next
- 後で (あとで) — later
- 前 (まえ) — before, in front
- 後 (あと) / うしろ — after, behind

## 13. Locations and Places (general)

- ところ — place
- いえ / うち — house, home
- へや — room
- だいどころ — kitchen
- おてあらい — restroom
- トイレ — toilet
- おふろ — bath
- げんかん — entrance
- にわ — garden
- 学校 (がっこう) — school
- 大学 (だいがく) — university
- 高校 (こうこう) — high school
- きょうしつ — classroom
- としょかん — library
- びょういん — hospital
- ぎんこう — bank
- ゆうびんきょく — post office
- 会社 (かいしゃ) — company
- じむしょ — office
- お店 (おみせ) — shop
- スーパー — supermarket
- デパート — department store
- きっさてん — café
- レストラン — restaurant
- やおや — greengrocer
- ほんや — bookstore
- はなや — flower shop
- にくや — butcher
- パンや — bakery
- 駅 (えき) — station
- くうこう — airport
- こうえん — park
- どうぶつえん — zoo **[Ext]**
- びじゅつかん — art museum **[Ext]**
- えいがかん — movie theater
- ホテル — hotel
- りょかん — Japanese inn **[Cul]**
- たいしかん — embassy **[Ext]**
- こうばん — police box **[Cul]**
- こうじょう — factory
- こうさてん — intersection
- いりぐち — entrance, entry
- しょくどう — cafeteria, dining room
- たてもの — building
- ろうか — hallway, corridor
- プール — swimming pool
- ポスト — post (mailbox)
- 道 (みち) — road, way
- とおり — street
- かど — corner
- はし — bridge
- まち — town
- むら — village
- 国 (くに) — country
- 外 (そと) — outside
- 中 (なか) — inside, middle
- 上 (うえ) — top, above
- 下 (した) — under, below
- 前 (まえ) — front
- 後ろ (うしろ) — back, behind
- 左 (ひだり) — left
- 右 (みぎ) — right
- となり — next to
- よこ — side
- ちかく — near
- とおく — far
- むこう — over there, beyond
- 北 (きた) — north
- 南 (みなみ) — south
- 東 (ひがし) — east
- 西 (にし) — west

## 14. Nature and Weather

- 山 (やま) — mountain
- 川 (かわ) — river
- うみ — sea, ocean
- いけ — pond
- みずうみ — lake
- もり — forest
- 木 (き) — tree
- 花 (はな) — flower
- くさ — grass
- は — leaf
- いし — stone
- 田 (た) — rice field
- 空 (そら) — sky
- くも — cloud
- たいよう — sun
- 月 (つき) — moon
- ほし — star
- 雨 (あめ) — rain
- ゆき — snow
- かぜ — wind
- 天気 (てんき) — weather
- はれ — clear weather
- くもり — cloudy
- はる — spring
- なつ — summer
- あき — autumn
- ふゆ — winter
- あつい — hot (weather)
- さむい — cold (weather)
- すずしい — cool
- あたたかい — warm
- 火 (ひ) — fire
- 水 (みず) — water (cold)
- おゆ — hot water

## 15. Animals

- どうぶつ — animal
- いぬ — dog
- ねこ — cat
- とり — bird
- さかな — fish
- うま — horse
- うし — cow
- ぶた — pig
- にわとり — chicken
- ぞう — elephant **[Cul]**
- むし — insect

## 16. Food and Drink — General

- たべもの — food
- のみもの — drink
- あさごはん — breakfast
- ひるごはん — lunch
- ばんごはん — dinner
- ゆうはん — dinner (alternate)
- ごはん — rice, meal
- しょくじ — meal
- おべんとう — boxed lunch
- おかし — sweets, snacks

## 17. Food — Items

- パン — bread
- たまご — egg
- にく — meat
- ぎゅうにく — beef
- ぶたにく — pork
- とりにく — chicken (meat)
- さかな — fish
- やさい — vegetable
- くだもの — fruit
- りんご — apple
- みかん — mandarin orange
- バナナ — banana
- いちご — strawberry
- ぶどう — grape
- すいか — watermelon
- レモン — lemon
- だいこん — daikon radish **[Cul]**
- にんじん — carrot
- たまねぎ — onion
- じゃがいも — potato
- トマト — tomato
- きゅうり — cucumber
- キャベツ — cabbage
- こめ — uncooked rice
- しお — salt
- さとう — sugar
- しょうゆ — soy sauce **[Cul]**
- みそ — miso **[Cul]**
- バター — butter
- チーズ — cheese
- すし — sushi **[Cul]**
- 天ぷら (てんぷら) — tempura **[Cul]**
- カレー — curry
- ラーメン — ramen
- うどん — udon **[Cul]**
- そば — soba **[Cul]**
- ハンバーガー — hamburger
- サンドイッチ — sandwich
- サラダ — salad
- スープ — soup
- ケーキ — cake
- アイスクリーム — ice cream
- チョコレート — chocolate
- あめ — candy (homophone of 雨 "rain")

## 18. Drinks

- 水 (みず) — water
- おゆ — hot water
- おちゃ — tea, green tea (Japanese tea)
- こうちゃ — black tea
- コーヒー — coffee
- ぎゅうにゅう — milk (also commonly: ミルク)
- ジュース — juice
- おさけ — alcohol, sake
- ビール — beer
- ワイン — wine

## 19. Tableware and Cooking

- さら — plate
- おさら — plate (polite)
- ちゃわん — rice bowl
- おわん — bowl
- はし — chopsticks
- スプーン — spoon
- フォーク — fork
- ナイフ — knife
- コップ — cup, glass
- カップ — cup (alt of コップ)
- れいぞうこ — refrigerator
- なべ — pot, pan

## 20. Colors

- いろ — color
- 白 (しろ) — white (noun)
- 白い (しろい) — white (adj)
- くろ — black (noun)
- くろい — black (adj)
- あか — red (noun)
- あかい — red (adj)
- あお — blue (noun)
- あおい — blue (adj)
- きいろ — yellow (noun)
- きいろい — yellow (adj)
- ちゃいろ — brown
- みどり — green
- ピンク — pink

## 21. Clothing and Accessories

- ふく — clothes
- ようふく — Western clothes
- きもの — kimono, traditional clothes **[Cul]**
- うわぎ — jacket, coat
- コート — coat
- セーター — sweater
- シャツ — shirt
- Tシャツ (ティーシャツ) — T-shirt
- ワイシャツ — dress shirt
- ズボン — trousers
- スカート — skirt
- ネクタイ — necktie
- ぼうし — hat, cap
- くつ — shoes
- くつした — socks
- ハンカチ — handkerchief
- かばん — bag
- さいふ — wallet
- めがね — glasses
- とけい — watch, clock
- ボタン — button
- ポケット — pocket
- かさ — umbrella

## 22. Money and Shopping

- お金 (おかね) — money
- 円 (えん) — yen
- ドル — dollar
- ねだん — price
- きっぷ — ticket
- きって — postage stamp
- はがき — postcard
- ふうとう — envelope
- てがみ — letter
- にもつ — luggage, package
- おみやげ — souvenir **[Cul]**
- レジ — register, cashier

## 23. Transport

- 車 (くるま) — car
- じどうしゃ — automobile
- じてんしゃ — bicycle
- バイク — motorbike
- バス — bus
- タクシー — taxi
- 電車 (でんしゃ) — train (electric)
- きしゃ — train (steam, archaic but still in older N5 lists) **[Ext]**
- ちかてつ — subway
- ひこうき — airplane
- ふね — ship, boat
- 道 (みち) — road
- しんごう — traffic light, signal

## 24. School and Study

- べんきょう — study
- じゅぎょう — class, lesson
- しゅくだい — homework
- しけん — exam
- テスト — test
- しつもん — question
- こたえ — answer
- いみ — meaning
- ことば — word, language
- じ — character, letter
- かんじ — kanji, Chinese characters
- かな — kana
- ひらがな — hiragana
- カタカナ — katakana
- もじ — letter, character
- ぶん — sentence
- ぶんしょう — composition, text
- ぶんぽう — grammar
- れい — example
- れんしゅう — practice
- 本 (ほん) — book
- きょうかしょ — textbook
- じしょ — dictionary
- ざっし — magazine
- 新聞 (しんぶん) — newspaper
- かみ — paper
- えんぴつ — pencil
- ボールペン — ballpoint pen
- まんねんひつ — fountain pen **[Ext]**
- ペン — pen
- ノート — notebook
- こくばん — blackboard
- チョーク — chalk
- つくえ — desk
- いす — chair
- けしゴム — eraser
- ちず — map
- しゃしん — photograph
- え — picture, drawing
- 番号 (ばんごう) — number
- 電気 (でんき) — electricity, light
- 電話 (でんわ) — telephone
- 電話番号 (でんわばんごう) — phone number

## 25. Languages and Countries

- 日本 (にほん / にっぽん) — Japan
- 日本語 (にほんご) — Japanese (language)
- 日本人 (にほんじん) — Japanese (person)
- アメリカ — America
- えいご — English (language)
- 中国 (ちゅうごく) — China
- 中国語 (ちゅうごくご) — Chinese (language)
- かんこく — Korea
- かんこくご — Korean (language)
- フランス — France
- フランスご — French (language)
- ドイツ — Germany
- スペイン — Spain
- イギリス — United Kingdom
- 外国 (がいこく) — foreign country
- 外国語 (がいこくご) — foreign language

## 26. House and Furniture

- いえ / うち — house, home
- アパート — apartment
- マンション — condominium **[Cul]**
- ドア — door
- と — door (Japanese-style)
- もん — gate
- まど — window
- かべ — wall
- かいだん — stairs
- エレベーター — elevator
- へや — room
- げんかん — entrance
- いま — living room
- しんしつ — bedroom
- ベッド — bed
- ふとん — futon **[Cul]**
- もうふ — blanket
- まくら — pillow
- テーブル — table
- つくえ — desk
- いす — chair
- たな — shelf
- ほんだな — bookshelf
- カーテン — curtain
- かぎ — key, lock
- 電気 (でんき) — light
- せっけん — soap
- はブラシ — toothbrush
- タオル — towel
- テープ — tape
- テレビ — TV
- ラジオ — radio
- カメラ — camera
- ビデオ — video
- えいが — movie
- おんがく — music
- うた — song
- え — picture
- ピアノ — piano
- ギター — guitar

## 27. Verbs — Group 1 (う-verbs)

- 会う (あう) — to meet
- 言う (いう) — to say
- 行く (いく) — to go
- うたう — to sing
- おもう — to think
- 買う (かう) — to buy
- 書く (かく) — to write
- 聞く (きく) — to listen, to ask
- きる — to cut
- つくる — to make
- しる — to know
- すむ — to live (reside)
- 立つ (たつ) — to stand
- とる — to take
- とる — to take (a photo) (variant of above)
- なく — to cry (animals)
- 飲む (のむ) — to drink
- 入る (はいる) — to enter
- はく — to put on (lower body)
- 話す (はなす) — to speak
- はしる — to run
- はたらく — to work
- まつ — to wait
- もつ — to hold, to have
- 休む (やすむ) — to rest
- 読む (よむ) — to read
- わたる — to cross
- 分かる (わかる) — to understand
- おわる — to end
- はじまる — to begin (intransitive)
- かえる — to return home
- うる — to sell
- おす — to push
- およぐ — to swim
- ひく — to play (instrument)
- ひく — to pull (variant of above)
- よぶ — to call
- とぶ — to fly, to jump
- こまる — to be troubled **[Ext]**
- ならぶ — to line up **[Ext]**
- のぼる — to climb **[Ext]**
- わたす — to hand over **[Ext]**
- ぬぐ — to take off (clothes)
- あらう — to wash
- いそぐ — to hurry
- しぬ — to die
- すう — to breathe in, to smoke
- ちがう — to differ
- つかう — to use
- つく — to arrive
- ならう — to learn
- はる — to stick, to paste
- まがる — to turn
- みがく — to polish, to brush **[Ext]**
- もっていく — to take (something)
- もってくる — to bring (something)
- あく — to open (intransitive; pair with あける)
- しまる — to close (intransitive; pair with しめる)
- だす — to take out, to put out (transitive; pair with でる)
- おとす — to drop (transitive; pair with おちる)
- ふく — to blow (of the wind)
- ふる — to fall (rain, snow)
- くもる — to become cloudy
- なくす — to lose (something)
- のる — to get on (train, bus, plane)
- すわる — to sit
- たのむ — to ask (a favor)
- とまる — to stop (intransitive)
- つとめる — to work for / be employed
- おく — to place, to put
- さく — to bloom
- かかる — to take (time / money)
- さす — to put up (an umbrella), to raise

## 28. Verbs — Group 2 (る-verbs)

- いる — to exist (animate)
- 食べる (たべる) — to eat
- 見る (みる) — to see, to watch
- ねる — to sleep
- おきる — to get up
- 出る (でる) — to leave, to go out
- 入れる (いれる) — to put in
- あける — to open (transitive)
- しめる — to close (transitive)
- おしえる — to teach
- おぼえる — to remember
- わすれる — to forget
- かりる — to borrow
- こたえる — to answer
- 出かける (でかける) — to go out
- かける — to call (phone), to wear (glasses)
- きる — to wear (upper body)
- つける — to switch on
- けす — to turn off, to erase
- ならべる — to line up (transitive) **[Ext]**
- はじめる — to begin (transitive)
- 見せる (みせる) — to show
- あびる — to bathe (shower)
- いれる — to make (tea / coffee) (note: same form as 入れる; context distinguishes)
- あつめる — to collect
- きえる — to go off, to disappear (intransitive; pair with けす)
- おちる — to fall (intransitive; pair with おとす)
- はれる — to be sunny / clear up
- つかれる — to get tired
- 生まれる (うまれる) — to be born
- おりる — to get off (train, bus)
- しめる — to tie / fasten / tighten (distinct from しめる "to close")

## 29. Verbs — Irregular and する-verbs

- する — to do
- 来る (くる) — to come
- べんきょうする — to study
- けっこんする — to get married
- さんぽする — to take a walk
- りょこうする — to travel
- れんしゅうする — to practice
- しつもんする — to ask a question
- しごとする — to work
- 電話する (でんわする) — to telephone
- コピーする — to copy
- そうじする — to clean
- せんたくする — to do laundry
- かいものする — to go shopping

## 30. Verbs — Existence and Possession

- ある — to exist (inanimate)
- いる — to exist (animate)
- いる — to need (different verb; same form)
- ござる / ございます — to be (very polite)
- やる — to give (down / casual)
- あげる — to give
- もらう — to receive
- くれる — to give (to me / us)
- かす — to lend
- かりる — to borrow
- かえす — to return (something)

## 31. い-Adjectives

- 大きい (おおきい) — big
- 小さい (ちいさい) — small
- 新しい (あたらしい) — new
- 古い (ふるい) — old (not for people)
- いい / よい — good
- わるい — bad
- あつい — hot (weather)
- さむい — cold (weather)
- あつい — hot (to touch) (variant of "あつい" hot-weather above; same reading, different sense)
- つめたい — cold (to touch)
- 高い (たかい) — high, tall, expensive
- ひくい — low, short (height)
- 安い (やすい) — cheap
- 長い (ながい) — long
- みじかい — short (length)
- ひろい — wide, spacious
- せまい — narrow
- おもい — heavy
- かるい — light (weight)
- あつい — thick (variant of "あつい"; same reading, different sense)
- うすい — thin
- ふとい — thick (round things)
- ほそい — thin (round things)
- つよい — strong
- よわい — weak
- はやい — early / fast (one reading covers both senses)
- おそい — slow, late
- たのしい — fun, enjoyable
- うれしい — happy
- かなしい — sad
- さびしい — lonely
- いたい — painful
- かわいい — cute
- うつくしい — beautiful
- きたない — dirty
- いそがしい — busy
- やさしい — easy / kind
- むずかしい — difficult
- おもしろい — interesting, funny
- つまらない — boring
- おいしい — delicious
- まずい — tastes bad
- あまい — sweet
- からい — spicy, salty
- にがい — bitter
- おおい — many
- すくない — few
- ちかい — near
- とおい — far
- あかるい — bright
- くらい — dark
- あたたかい — warm
- すずしい — cool
- まるい — round
- しかくい — square
- わかい — young
- きいろい — yellow
- あおい — blue
- あかい — red
- くろい — black
- 白い (しろい) — white
- ちゃいろい — brown
- あぶない — dangerous
- ほしい — want (something)
- ぬるい — lukewarm
- うるさい — noisy, annoying

## 32. な-Adjectives

- げんき — healthy, energetic
- しずか — quiet
- にぎやか — lively, bustling
- きれい — pretty, clean
- しんせつ — kind
- ひま — free time
- たいへん — tough, difficult
- だいじょうぶ — alright, OK
- べんり — convenient
- ふべん — inconvenient
- ゆうめい — famous
- じょうず — skilled
- へた — unskilled
- すき — liked
- きらい — disliked
- だいすき — love
- だいきらい — hate
- おなじ — same
- いろいろ — various
- りっぱ — splendid **[Ext]**
- かんたん — easy, simple
- けっこう — fine, sufficient
- たいせつ — important
- だいじ — important
- あんぜん — safe
- じょうぶ — sturdy
- いや — unpleasant, disagreeable

## 33. Adverbs

- とても — very
- すごく — very (casual)
- すこし — a little
- ちょっと — a little
- たくさん — many, a lot
- おおぜい — a lot of (people)
- だいたい — generally, roughly
- ぜんぶ — all
- みんな / みな — everyone
- もう — already
- まだ — still, not yet
- もうすこし — a little more
- もっと — more
- 一番 (いちばん) — most, number one
- とくに — especially
- ほんとうに — truly, really
- ゆっくり — slowly
- すぐ — soon
- たぶん — probably
- いっしょに — together
- 一人で (ひとりで) — alone
- じぶんで — by oneself
- かならず — surely, definitely
- きっと — surely
- もちろん — of course
- どうぞ — please (offering)
- どうも — thanks, indeed
- どうぞよろしく — please treat me well
- どう — how
- なぜ — why
- どうして — why
- まっすぐ — straight ahead
- もういちど — once more
- まず — first of all
- では / じゃ — well then
- もしもし — hello (telephone)

## 34. Conjunctions

- そして — and then
- それから — and then, after that
- それで — and so
- でも — but, however
- しかし — but, however
- けれど / けれども / けど — but
- が — but (clause-connector)
- から — because, so
- だから — therefore
- ですから — therefore (polite)
- それに — moreover
- じゃあ / では — well then
- ところで — by the way
- または — or

## 35. Particles (functional vocabulary)

- は — topic marker
- が — subject marker
- を — object marker
- に — to, at, in (target / time)
- で — at, by, with (location of action / means)
- へ — to (direction)
- と — and, with
- か — question marker
- も — also
- や — and (non-exhaustive)
- の — of, possessive / nominalizer
- から — from, because
- まで — until
- ね — sentence-final (agreement)
- よ — sentence-final (assertion)
- だけ — only
- しか — only (with negative)
- ぐらい / くらい — about
- ごろ — around (time)
- ずつ — each
- など — etc.

## 36. Greetings and Set Phrases

- おはようございます — good morning
- こんにちは — hello / good afternoon
- こんばんは — good evening
- おやすみなさい — good night
- さようなら — goodbye
- しつれいします — excuse me / pardon me
- しつれいしました — excuse me (past)
- ありがとう — thanks
- ありがとうございます — thank you
- どういたしまして — you're welcome
- すみません — excuse me / sorry
- ごめんなさい — I'm sorry
- おねがいします — please
- いただきます — said before eating
- ごちそうさまでした — said after eating **[Cul]**
- いってきます — I'm leaving (home) **[Cul]**
- いってらっしゃい — see you / take care **[Cul]**
- ただいま — I'm home **[Cul]**
- おかえりなさい — welcome back **[Cul]**
- はじめまして — nice to meet you
- どうぞよろしく — pleased to meet you
- おげんきですか — how are you?
- おかげさまで — thanks to you **[Cul]**
- いらっしゃいませ — welcome (to a shop) **[Cul]**
- どうぞ — please (offering)
- どうも — thanks
- もしもし — hello (phone)

## 37. Common Nouns — Miscellaneous

- もの — thing
- こと — thing (intangible)
- 名前 (なまえ) — name
- ことば — word
- 話 (はなし) — story, talk
- しごと — work, job
- やくそく — promise, appointment
- ようじ — errand
- もんだい — problem
- しゅみ — hobby
- りょこう — travel
- さんぽ — walk
- うんどう — exercise
- ゲーム — game
- スポーツ — sport
- しあい — match, game
- ニュース — news
- パーティー — party
- きって — stamp
- はがき — postcard
- てがみ — letter
- きっぷ — ticket
- おみやげ — souvenir (also in §22 — listed here for thematic completeness) **[Cul]**
- プレゼント — present, gift
- りゅうがく — study abroad **[Ext]**
- けっこん — marriage
- しゃしん — photograph
- りょかん — Japanese inn (also in §13) **[Cul]**
- かぜ — cold (illness)
- びょうき — illness
- くすり — medicine
- けが — injury
- おゆ — hot water
- おふろ — bath (also in §13)
- シャワー — shower
- マッチ — match (lighting) **[Cul]**
- たばこ — cigarette
- はいざら — ashtray **[Cul]**
- スリッパ — slippers
- ティッシュ — tissue
- フィルム — film **[Cul]**
- レコード — record **[Cul]**
- テープ — tape (also in §26)
- よてい — plan, schedule
- じかんわり — timetable
- はこ — box
- はんぶん (半分) — half
- はたち — 20 years old (special reading)
- へん — area, vicinity
- ほか — other (place / thing / person)
- ほんとう — truth, reality
- なつやすみ — summer vacation
- やすみ — rest, holiday (noun form of やすむ)
- りょうり — cuisine, cooking
- ペット — pet
- カレンダー — calendar
- かてい — household, home life
- かびん — vase
- かた — way of doing (e.g., たべかた = how to eat)
- おくさん — wife (someone else's; polite)
- 先 (さき) — earlier, ahead, previous
- せびろ — business suit (older term, replaced by スーツ in modern usage) **[Ext]**
- 大きな (おおきな) — big (variant adjectival form of 大きい)
- たて — length, height
- ゆうべ — last night
- にっき — diary, journal
- さくぶん — composition, written essay
- じびき — dictionary (older term, alt for じしょ) **[Ext]**
- テープレコーダー — tape recorder **[Cul]**
- ストーブ — heater, stove **[Cul]**
- ページ — page
- クラス — class
- グラム — gram
- メートル — metre, meter
- キログラム — kilogram
- キロメートル — kilometre, kilometer

## 38. Sounds and Voice

- こえ — voice
- おと — sound
- うた — song

## 39. Function / Filler Expressions

- えーと — um, let me see
- あの — um (hesitation)
- そうですね — let me think / that's right
- そうですか — is that so?
- いいえ — no
- はい — yes
- ええ — yes (casual)
- うん — yeah (informal)
- ううん — no (informal)
- やはり / やっぱり — as expected
- さあ — well... (filler / hesitation)
- いかが — how (polite form of どう)
- それでは — in that case, well then

## 40. Misc Useful Items

- もの — thing (physical) (also in §37)
- こと — thing (abstract) (also in §37)
- ばしょ — place
- ばあい — case, situation
- ほう — direction, side
- とき — time, occasion (also in §10)
- 番号 (ばんごう) — number (also in §24)
- じゅうしょ — address
- 名前 (なまえ) — name (also in §37)
- ねんれい — age
- しごと — job (also in §37)
- 学校 (がっこう) — school (also in §13)
- しゅみ — hobby (also in §37)
- しゅっしん — hometown / origin

---

## Notes on Coverage

- This list intentionally cross-lists some items (e.g., 名前, しごと, おみやげ) under multiple thematic sections. Each cross-listing is annotated; they are not accidental duplicates.
- Items tagged **[Ext]** or **[Cul]** are kept for completeness but should be deprioritized when planning a focused N5 study schedule. Strip the `[Ext]` and `[Cul]` lines for a tighter "exam-only" subset.
- Kanji rendering follows the rule stated above: only kanji from `kanji_n5.md` appear; everything else is in hiragana / katakana.
