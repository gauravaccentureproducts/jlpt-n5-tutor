...
**Answer: 1** - コーヒー is something one drinks (のむ).
...
**Answer: 1** - つかれた + やすむ. つかれましたから (polite past + から) is the N5-canonical reason -> action chain (replaces ので which leans N4 in major textbooks).
...
**Answer: 4** - turn at the corner (かど).
...
**Answer: 4** - こと + ある (experience). Common error: 「〜たことがある」 cannot combine with specific time markers (きょねん, etc.) - it expresses indefinite past experience. Use a plain past tense (きょねん 行きました) for time-specific events instead.
...
**Answer: 4** - take off shoes (before entering).
...
**Answer: 4** - 「病院で はたらく」 ≈ 「いしゃです」. N5 pragmatic substitution (working at a hospital is the standard textbook paraphrase of "is a doctor", though strictly someone could work at a hospital without being a doctor - nurse, admin). Tests the N5 vocab triangle 病院 / はたらく / いしゃ; replaces the prior tautological 「父は医者 = 父の仕事は医者」 which tested no vocabulary.
