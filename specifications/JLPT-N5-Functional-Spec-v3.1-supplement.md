...
|---|---|---|---|---|---|
| OQ-1 | Should we add a recommendation engine that runs on-device (privacy-preserving)? | **Deferred to v2.0** | Project author | **2026-09-30** | Constraint: any recommender must run client-side only (no telemetry, Hard constraint #2). Required: design pure-on-device weighting over LocalStorage history. Decision-blocking dependency: full Pass-11 audit completes (provides the cleaned content corpus the recommender would index). Re-evaluate in Q3 after content surface is stable. |
| OQ-2 | Should listening corpus expand from 12 → 30+ items? | **Deferred to v1.6** | Content owner + Pass-11 reviewer | **2026-07-30** (next quarterly gate) | Decision-blocking dependency: Pass-11 listening review (P3 in brief, currently 6/12 items reviewed). Once full P3 review completes, expand corpus to 24-30 items in same release if quality bar holds. |
| OQ-3 | Should we add a CSP meta tag for additional XSS hardening? | **Approved for v1.6** | Engineering | **2026-05-15** | Low risk (no inline scripts; no third-party CDN). Add `<meta http-equiv="Content-Security-Policy" content="default-src 'self'; img-src 'self' data:; media-src 'self'; style-src 'self' 'unsafe-inline'">` to `index.html`. `'unsafe-inline'` for styles is needed by the inline `<style>` blocks; revisit later. Implementation: ~30 minutes. Add Lighthouse Best Practices verification post-merge. |
...
| OQ-6 (NEW) | Should we publish a Japanese-language version of the brief / app? | **Open** | Project author + Content reviewer | **2026-07-30** | Raised by Pass-11 reviewer in finding B.10. Brief translation: high-value (~4 hours, attracts in-Japan reviewers, supports MEXT-alignment narrative). App translation: low-value at current learner-base; defer. **Recommended action:** translate the brief by 2026-07-30; defer app translation until learner-base data justifies it. |
...
| Security | HTTPS only (GitHub Pages). All `target="_blank"` links carry `rel="noopener noreferrer"`. CSP recommended (OQ-3). |
...
- **F-1** Listening corpus expansion (12 → 30+ items). Open question OQ-2.
- **F-2** Privacy-preserving on-device recommendation engine (no telemetry). OQ-1.
- **F-3** Optional CSP meta tag for additional XSS hardening. OQ-3.
